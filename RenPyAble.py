	...
	for f in file_list:
		...
		if operator.contains(f,"AFRONT"):			# run time: 1 - 0.314, 2 - 0.115
			...
			if f.endswith("FRONT",0, len(f)-4):
				new_name = f[:-9]+"A"+f[-9:]
				# This line is to prevent error when file already exist and can't be replaced
                # Often after files are printed there are some amendments to couple of files
                # And this files must be regenerated
                # When program tried to rename this error 1843 was rise
                # this line prevents this error
				if os.path.exists(new_name):
					os.remove(new_name)
